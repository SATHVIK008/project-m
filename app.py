...
def send_email(to_email, pdf_path):
    ...
    with open(pdf_path, "rb") as f:
        ...
        part['Content-Disposition'] = f'attachment; filename="Interview_Analysis_Report.pdf"'
        msg.attach(part)
...
# Collect candidate's name and email
if 'name' not in st.session_state or 'email' not in st.session_state:
    ...
    if 'questions' in st.session_state:
        ...
        for i, question_data in enumerate(st.session_state.questions):
            ...
            components.html(f"""
                <div>
                    <video id="video_{i}" width="320" height="240" controls></video>
                    <br>
                    <button id="record_{i}">Record</button>
                    <button id="stop_{i}">Stop</button>
                    <script>
                        let video = document.getElementById('video_{i}');
                        let recordButton = document.getElementById('record_{i}');
                        let stopButton = document.getElementById('stop_{i}');
                        let mediaRecorder;
                        let recordedChunks = [];
            ...
            video_file = st.file_uploader(f"Upload video answer for Question {i + 1}", type=["mp4"], key=f"uploader_{i}")
            
            if video_file and not question_data["transcribed"]:
                with st.spinner("Transcribing video..."):
                    video_path = f"uploaded_video_{i + 1}.mp4"
                    with open(video_path, "wb") as f:
                        f.write(video_file.read())
                    transcript = transcribe_video(video_path)
                    st.session_state.questions[i]["answer"] = transcript
                    st.session_state.questions[i]["transcribed"] = True
